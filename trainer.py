...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/pr-net.yaml', help='Path to the config file.')
    ...
    # Start training
    iterations = trainer.resume(checkpoint_directory,need_opt=True) if opts.resume else 0
    ...
    while True:
        for it,out_data  in enumerate(train_loader):
            ...
            with Timer("Elapsed time in update: %f"):
                # Main training code
                trainer.gen_update_rotate(Xa_out, Xb_out, light_a_label, light_b_label, r_Xb_out, mask, diff)
